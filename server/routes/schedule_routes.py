...
class ScheduleCollectionAPI(MethodView):
    REQUIRED_FIELDS = (
        "folderId", "folderName", "cardIds", "scheduleType", "time",
        "durationMinutes", "intervalMinutes", "createdAt",
    )

    def get(self):
        schedules = [schedule.to_dict() for schedule in schedule_service.list_all()]
        return jsonify({"success": True, "data": schedules, "message": "Data fetched successfully."})
...
schedules_bp.add_url_rule("/schedules", view_func=ScheduleCollectionAPI.as_view("schedule_collection"))
schedules_bp.add_url_rule("/schedules/<string:schedule_id>", view_func=ScheduleItemAPI.as_view("schedule_item"))
