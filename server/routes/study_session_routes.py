...
def parse_timestamp(value: object, field_name: str):
    if not isinstance(value, str) or not value:
        return None, (
            jsonify({"success": False, "message": f"{field_name} is required"}),
            400,
        )
    try:
        ...
        return None, (
            jsonify({"success": False, "message": f"Invalid {field_name} format"}),
            400,
        )
...
class StudySessionStartAPI(MethodView):
    ...
    def post(self):
        ...
        try:
            ...
            return (
                jsonify({"success": False, "message": "Failed to start study session"}),
                500,
            )

        current_app.logger.info(
            "Study session %s: %s", "created" if created else "reused", session.id
        )
        return jsonify(
            {
                "success": True,
                "message": (
                    ...
                    if created
                    else "Study session already active"
                ),
...
class StudySessionEndAPI(MethodView):
    ...
    def post(self):
        ...
        if session_id is not None and (
            not isinstance(session_id, int) or isinstance(session_id, bool)
        ):
            return (
                jsonify({"success": False, "message": "sessionId must be an integer"}),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify({"success": False, "message": "Failed to end study session"}),
                500,
            )

        current_app.logger.info(
            "Study session updated: %s (%s seconds)",
            session.id,
...
studysession_bp.add_url_rule(
    ...
    view_func=StudySessionStartAPI.as_view("study_session_start"),
)
studysession_bp.add_url_rule(
    "/study-sessions/end", view_func=StudySessionEndAPI.as_view("study_session_end")
)
