...
class FolderCollectionAPI(MethodView):
    ...
    def get(self):
        ...
        folders = [folder.to_dict() for folder in folder_service.list_all(user_id)]
...
class FolderItemAPI(MethodView):
    ...
    def delete(self, folder_id: str):
        ...
        current_app.logger.info(
            "DELETE folder requested: folder_id=%s current_user.id=%s",
            folder_id,
            user_id,
        )
        try:
            ...
            current_app.logger.info(
                "DELETE folder lookup: folder_id=%s found=false", folder_id
            )
            raise
        current_app.logger.info(
...
folders_bp.add_url_rule(
    "/folders", view_func=FolderCollectionAPI.as_view("folder_collection")
)
folders_bp.add_url_rule(
    "/folders/<string:folder_id>", view_func=FolderItemAPI.as_view("folder_item")
)
